...
function normalizeErrorLike(errorLike: any) {
...
function recordException(error: any, options?: any) {
...
function installProcessErrorHandlers(config: any, logger: any, shutdownObservability: any) {
  const listeners = [];
  ...
  async function handleFatal(eventName, errorLike) {
    ...
    try {
      ...
      process.stderr.write(
        'observability shutdown error during ' + eventName + ': ' + shutdownError.message + '\n'
      );
    } finally {
      process.exit(1);
    }
