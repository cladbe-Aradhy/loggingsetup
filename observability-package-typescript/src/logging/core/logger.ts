...
function shouldMirror(config: any) {
...
function writeJsonLine(level: string, payload: any) {
  const line = JSON.stringify(payload) + '\n';
  const stream = level === 'error' || level === 'warn' ? process.stderr : process.stdout;
  stream.write(line);
}

function toOtelAttributes(record: Record<string, any>) {
...
function annotateActiveSpan(level: string, message: any, errorValue: any) {
...
function writeRecordToOutputs(state: any, config: any, level: string, record: any) {
...
function createLogger(config: any, state: any, bindings?: Record<string, any>) {
  ...
  function emit(level: string, message: any, fields?: any, meta?: any) {
