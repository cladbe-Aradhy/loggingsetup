...
const { buildSummaryMessage, buildDedupeKey, flushPendingLogDedupe, registerLogForDedupe, shouldDedupeLog } = require('../src/logging/core/dedupe');
...
test('buildSummaryMessage creates a UI-friendly summary body', () => {
    assert.equal(buildSummaryMessage('DB failed', 4, 5), 'DB failed (repeated 4 times, total 5)');
    assert.equal(buildSummaryMessage('', 1, 2), 'repeated log (repeated 1 time, total 2)');
});
