...
function detectPreloadedInstrumentedModules() {
    ...
    const moduleLoadList = (process.moduleLoadList || []);
...
async function performInit(options) {
    ...
    if (state.preloadedModules.length > 0) {
        process.stderr.write('observability startup warning: some instrumented modules were loaded before initObservability: ' +
    ...
    state.errorRestore = getErrorsModule().installProcessErrorHandlers(config, state.appLogger, shutdownObservability);
...
function buildExpressApi() {
    const { createRequestContextMiddleware, createRequestLoggingMiddleware, createErrorMiddleware } = getExpressModule();
    return {
