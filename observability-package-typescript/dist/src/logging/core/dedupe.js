...
function buildSummaryMessage(message, repeatCount, totalCount) {
...
function shouldDedupeLog(config, level, meta) {
...
function buildDedupeKey(record, level) {
...
function scheduleFlush(state, key, windowMs, emitSummary) {
...
function flushEntry(state, key, emitSummary) {
    const entry = state.logDedupeEntries.get(key);
    if (!entry) {
        return;
    }
...
function registerLogForDedupe(state, config, level, record, emitSummary) {
...
function flushPendingLogDedupe(state, emitSummary) {
